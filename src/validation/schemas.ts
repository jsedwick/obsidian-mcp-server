/**
 * Zod validation schemas for all 27 MCP tool inputs
 *
...
export const ListRecentSessionsArgsSchema = z.object({
  ...
  _invoked_by_slash_command: z.boolean().optional().describe('Internal flag - set by slash commands only'),
});
...
export const SearchVaultArgsSchema = z.object({
  ...
  directories: z.array(z.string()).optional().describe('Directories to search (e.g., ["sessions", "topics"])'),
  max_results: PositiveNumber.optional().default(10).describe('Maximum number of results to return'),
  date_range: DateRangeSchema.describe('Optional date range filter'),
  snippets_only: z.boolean().optional().default(true).describe('Return condensed snippets instead of full matches'),
  detail: DetailLevelSchema.optional().default('summary').describe('Response detail level'),
});
...
export const EnhancedSearchArgsSchema = z.object({
  ...
  current_session_id: z.string().optional().describe('Optional session ID to use for contextual search'),
  max_results_per_query: PositiveNumber.optional().default(5).describe('Maximum results per query variation'),
});
...
export const ToggleEmbeddingsArgsSchema = z.object({
  enabled: z.boolean().optional().describe('Set to true to enable, false to disable (toggles if omitted)'),
});
...
export const CreateTopicPageArgsSchema = z.object({
  topic: NonEmptyString.min(3, 'Topic name must be at least 3 characters').describe('Topic name (will be slugified for filename)'),
  content: NonEmptyString.min(10, 'Content must be at least 10 characters').describe('Content for the topic page'),
  auto_analyze: z.union([
    z.boolean(),
    z.literal('smart')
  ]).optional().default(false).describe('Auto-analyze content for tags and metadata'),
});
...
export const UpdateTopicPageArgsSchema = z.object({
  ...
  append: z.boolean().optional().default(true).describe('If true, append to existing content; if false, replace'),
});
...
export const FindStaleTopicsArgsSchema = z.object({
  age_threshold_days: z.number().int().positive().optional().default(365).describe('Number of days since creation or last review to consider a topic stale'),
  include_never_reviewed: z.boolean().optional().default(true).describe('Include topics that have never been reviewed'),
});

// review_topic
export const ReviewTopicArgsSchema = z.object({
  topic: NonEmptyString.describe('Topic name or slug to review'),
  analysis_prompt: z.string().optional().describe('Optional custom instructions for the review analysis'),
});
...
export const ApproveTopicUpdateArgsSchema = z.object({
  ...
  modified_content: z.string().optional().describe('Optional: edited content if you want to modify the AI suggestion before applying'),
});
...
export const MigrateCommitBranchesArgsSchema = z.object({
  project_slug: z.string().optional().describe('Optional: Project slug to migrate (e.g., "obsidian-mcp-server")'),
  dry_run: z.boolean().optional().default(false).describe('If true, shows what would be changed without making changes'),
});
...
export const AnalyzeCommitImpactArgsSchema = z.object({
  ...
  include_diff: z.boolean().optional().default(false).describe('Include full diff in analysis (default: false, uses stat summary)'),
});
...
export const CreateDecisionArgsSchema = z.object({
  title: NonEmptyString.min(5, 'Decision title must be at least 5 characters').describe('Decision title'),
  content: NonEmptyString.min(20, 'Decision content must be at least 20 characters').describe('Decision content (rationale, alternatives, consequences)'),
  ...
  force: z.boolean().optional().default(false).describe('Set to true to bypass keyword detection warnings'),
});

// extract_decisions_from_session
export const ExtractDecisionsFromSessionArgsSchema = z.object({
  session_id: z.string().optional().describe('Optional session ID to analyze (defaults to current session)'),
  content: z.string().optional().describe('Optional content to analyze instead of reading from session file'),
});
...
export const VaultCustodianArgsSchema = z.object({
  files_to_check: z.array(AbsolutePath).optional().describe('Optional: Array of absolute file paths to check (checks all vault files if not provided)'),
});
 ...
 */
export type InferSchemaType<T extends keyof typeof ValidationSchemas> = z.infer<typeof ValidationSchemas[T]>;
