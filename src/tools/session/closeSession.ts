...
export async function closeSession(
    ...
    await Promise.all(
      repoCandidates.map(async c => {
        try {
          ...
          commitCounts.set(c.path, commits.length);
        } catch {
          commitCounts.set(c.path, 0);
        }
      })
    );
    qualifyingRepos = repoCandidates.filter(c => (commitCounts.get(c.path) ?? 0) > 0);
    logger.debug('qualifyingRepos:', {
      qualifying: qualifyingRepos.map(c => ({
        name: c.name,
        score: c.score,
        commits: commitCounts.get(c.path) ?? 0,
      })),
  ...
  if (qualifyingRepos.length > 0) {
    ...
    primary = topTier.reduce((best, c) =>
      (commitCounts.get(c.path) ?? 0) > (commitCounts.get(best.path) ?? 0) ? c : best
    );
