...
import {
  findUnrecordedRecentCommits,
  selectAllRepoCandidates,
  type DetectedRepo,
} from '../../utils/repoDetection.js';
...
export async function runPhase1Analysis(
  ...
  qualifyingRepos: DetectedRepo[] = [], // Decision 061: all repos with session-window commits
  commitsByRepo: Map<string, string[]> = new Map(), // Decision 061: hashes per candidate from auto-detect parallel scan
  repositorySlugs: string[] = [] // Decision 061: project slugs primary-first; mirrors frontmatter `repositories`
  ...
  if (detectedRepoInfo && sessionStartTime) {
    ...
    // Decision 061: reuse hashes from the parent's parallel scan when present
    // (auto-detect path always populates commitsByRepo for the primary).
    // Defensive fallback runs findSessionCommits only if the contract drifts.
    const tCommitsDetect = Date.now();
    const precomputed = commitsByRepo.get(detectedRepoInfo.path);
    if (precomputed !== undefined) {
      sessionCommits = precomputed;
    } else {
      try {
    ...
    logger.info('close.timing.phase1.commits_detect', {
...
`;
    ...
    logger.info('close.timing.phase1.commits_analyze', {
      elapsed_ms: Date.now() - tCommitsAnalyze,
      count: sessionCommits.length,
    });
  }
  ...
  logger.info('close.timing.phase1.semantic_review', {
    elapsed_ms: Date.now() - tSemanticReview,
    count: rawSemanticTopics.length,
  });
  ...
  const sessionData: SessionData = {
    ...
    monthDir,
    detectedRepoInfo,
    qualifyingRepos: qualifyingRepos.length > 0 ? qualifyingRepos : undefined,
    commitsByRepo:
      qualifyingRepos.length > 0
        ? Object.fromEntries(qualifyingRepos.map(r => [r.path, commitsByRepo.get(r.path) ?? []]))
        : undefined,
    repositorySlugs: repositorySlugs.length > 0 ? repositorySlugs : undefined,
  ...
  logger.info('close.timing.phase1.total', {
    elapsed_ms: Date.now() - phase1Start,
    commit_count: sessionCommits.length,
    semantic_review_count: semanticTopicsForReview.length,
  });
...
/**
 * Decision 061 step 6: when a session has commits from ≥2 repos, regroup the
 * flat `## Related Git Commits` section into per-repo `### {slug}` subheadings.
 * Subheading order follows `repositoriesOrder` (primary first); commits within
 * each subheading preserve their existing order. Single-repo sessions and
 * sections containing fewer than 2 distinct slugs are returned unchanged.
 *
 * Slug extraction parses the wikilink shape that `recordCommit` writes:
 * `- [[projects/{slug}/commits/{shortHash}|...]]: {subject}`. Lines that do
 * not match (e.g. hand-edited notes) are preserved at the bottom of the
 * section under no subheading so we never silently drop content.
 */
export function groupRelatedGitCommitsByRepo(
  sessionContent: string,
  repositoriesOrder: string[]
): string {
  if (repositoriesOrder.length < 2) return sessionContent;

  const header = '## Related Git Commits';
  const headerIdx = sessionContent.indexOf(header);
  if (headerIdx < 0) return sessionContent;

  // Section ends at the next H2 header, or EOF.
  const sectionStart = headerIdx + header.length;
  const afterHeader = sessionContent.slice(sectionStart);
  const nextH2RelIdx = afterHeader.search(/\n##\s/);
  const sectionEnd = nextH2RelIdx < 0 ? sessionContent.length : sectionStart + nextH2RelIdx;
  const body = sessionContent.slice(sectionStart, sectionEnd);

  // Bail out if section is already grouped (presence of `### ` subheadings) —
  // re-grouping would duplicate or reorder a hand-edited file.
  if (/^### /m.test(body)) return sessionContent;

  const slugLineRegex = /^- \[\[projects\/([^/\]]+)\/commits\//;
  const groups = new Map<string, string[]>();
  const unmatched: string[] = [];
  for (const line of body.split('\n')) {
    const m = line.match(slugLineRegex);
    if (m) {
      const slug = m[1];
  ...
  if (groups.size < 2) return sessionContent;

  const orderedSlugs = [
    ...repositoriesOrder.filter(s => groups.has(s)),
    ...Array.from(groups.keys())
      .filter(s => !repositoriesOrder.includes(s))
      .sort(),
  ];

  const groupedBody = orderedSlugs
    .map(slug => `### ${slug}\n${groups.get(slug)!.join('\n')}`)
    .join('\n\n');
  const finalBody =
    unmatched.length > 0 ? `${groupedBody}\n\n${unmatched.join('\n')}` : groupedBody;

  return (
    sessionContent.slice(0, headerIdx) +
    `${header}\n\n${finalBody}\n` +
    sessionContent.slice(sectionEnd)
  );
}

/**
 * Decision 061 step 6: file-IO wrapper around `groupRelatedGitCommitsByRepo`.
 * Returns the rewritten content when the file changed, or `null` when no
 * regrouping was needed (single-repo, missing slug list, or read failure).
 */
async function regroupMultiRepoCommitsSection(
  sessionFile: string,
  repositorySlugs: string[] | undefined
): Promise<string | null> {
  if (!repositorySlugs || repositorySlugs.length < 2) return null;
  try {
    const currentContent = await fs.readFile(sessionFile, 'utf-8');
    const regrouped = groupRelatedGitCommitsByRepo(currentContent, repositorySlugs);
    if (regrouped === currentContent) return null;
    await fs.writeFile(sessionFile, regrouped);
    return regrouped;
  } catch (err) {
    logger.debug('Step 6 regroup non-fatal', { error: String(err) });
    return null;
  }
}
...
export async function runPhase2Finalization(
  ...
  if (data.commitRelatedTopics && data.commitRelatedTopics.length > 0) {
    ...
    logger.info('close.timing.phase2.enforce_041', {
      elapsed_ms: Date.now() - tEnforce041,
      topic_count: data.commitRelatedTopics.length,
    });
  }
  ...
  if (data.semanticTopicsPresented && data.semanticTopicsPresented.length > 0) {
    ...
    logger.info('close.timing.phase2.enforce_042', {
      elapsed_ms: Date.now() - tEnforce042,
      topic_count: data.semanticTopicsPresented.length,
    });
  }
  ...
  logger.info('close.timing.phase2.session_write', {
    elapsed_ms: Date.now() - tSessionWrite,
  ...
  // Decision 061: when qualifyingRepos is populated, iterate every repo's
  // commits — not just the primary's. Falls back to the legacy single-repo
  // path for pre-061 SessionData shape and for sessions without
  // qualifyingRepos (auto-detect didn't run, or sessionStartTime missing).
  const reposToRecord: Array<{ repoPath: string; commits: string[] }> = [];
  if (data.qualifyingRepos && data.qualifyingRepos.length > 0 && data.commitsByRepo) {
    for (const repo of data.qualifyingRepos) {
      const commits = data.commitsByRepo[repo.path] ?? [];
      if (commits.length > 0) {
        reposToRecord.push({ repoPath: repo.path, commits });
      }
    }
  } else if (data.sessionCommits && data.sessionCommits.length > 0 && data.detectedRepoInfo) {
    reposToRecord.push({
      repoPath: data.detectedRepoInfo.path,
      commits: data.sessionCommits,
    });
  }
  for (const { repoPath, commits } of reposToRecord) {
  ...
  // Decision 061 step 6: when ≥2 repos qualified, regroup the flat
  // `## Related Git Commits` entries written by recordCommit into per-repo
  // `### {slug}` subheadings. Single-repo and edits-only sessions render
  // unchanged.
  const regrouped = await regroupMultiRepoCommitsSection(data.sessionFile, data.repositorySlugs);
  if (regrouped !== null) {
    data.sessionContent = regrouped;
  }
  ...
  if (allFilesToCheck.length > 0) {
    ...
    logger.info('close.timing.phase2.vault_custodian', {
      elapsed_ms: Date.now() - tCustodian,
      files_checked: allFilesToCheck.length,
    });
  }
  ...
  logger.info('close.timing.phase2.total', {
...
export interface SessionData {
  ...
  /**
   * Decision 061: every repo with ≥1 commit during the session window.
   * Sorted by detect_session_repositories score (descending). Primary
   * (`detectedRepoInfo`) is `qualifyingRepos[0]` when this list is non-empty,
   * else falls back to the top score candidate. Phase 2 iterates this list
   * to record commits per repo and render the multi-repo `## Related Git
   * Commits` subheadings. Empty list = single-repo session OR session with
   * no committed work (rendering unchanged from pre-061 behavior).
   */
  qualifyingRepos?: DetectedRepo[];
  /**
   * Decision 061: per-repo commit hashes from Phase 1's parallel scan.
   * Keyed by absolute repo path; values are session-window commit hashes
   * for that repo. Phase 2 iterates `qualifyingRepos` and looks up hashes
   * here to record per-repo. Absent on pre-061 SessionData (Phase 2 falls
   * back to the legacy single-repo `sessionCommits` path).
   */
  commitsByRepo?: Record<string, string[]>;
  /**
   * Decision 061: project slugs of every qualifying repo, primary first.
   * Same value as the session frontmatter `repositories` field; persisted in
   * SessionData so Phase 2 can drive the `## Related Git Commits` per-repo
   * subheading regrouping without recomputing slugs.
   */
...
export async function closeSession(
  ...
  if (args.finalize) {
    ...
    try {
      ...
      logger.info('close.timing.sync_vaults', {
        elapsed_ms: Date.now() - tSync,
        vault_count: context.allVaultPaths.length,
      });
  ...
  // Auto-detect Git repositories BEFORE building session content so the
  // project link is available in the session's Projects section. Decision 061:
  // capture every repo with session-window commits, not just the top scorer.

  // Compute sessionStartTime once — both qualifying-repo filter and Phase 1
  // commit detection consume it. Mirrors the fallback chain in runPhase1Analysis.
  let sessionStartTime = context.getSessionStartTime();
  if (!sessionStartTime && args.session_start_override) {
    try {
      ...
      // ignore — sessionStartTime stays null, qualifying filter becomes a no-op
    }
  }

  let detectedRepoInfo: { path: string; name: string; branch?: string; remote?: string } | null =
    null;
  let repoCandidates: DetectedRepo[] = [];
  let qualifyingRepos: DetectedRepo[] = [];

  // selectAllRepoCandidates handles all four priorities in Decision 037:
  //   0. detected_repo_override (returns single-element list, throws if non-Git)
  //   1. working_directories
  //   2. infer from files accessed
  //   3. fall back to MCP server cwd
  // Override-validation errors propagate (loud failure for caller intent);
  // detection errors fall through to silent recovery below.
  try {
    repoCandidates = await selectAllRepoCandidates({
    ...
    logger.debug('repoCandidates:', {
      candidates: repoCandidates.map(c => ({ name: c.name, path: c.path, source: c.source })),
    });
  } catch (err) {
    if (args.detected_repo_override) {
      // Bad explicit override — caller intent was loud, propagate.
      throw err;
    }
    // Auto-detect failure is non-fatal; close proceeds without a project link.
    logger.debug('selectAllRepoCandidates failed (auto-detect):', { error: String(err) });
  }

  // Decision 061: a repo qualifies if it has ≥1 commit during the session
  // window. Among qualifying repos at the same top score, c4ee373's tiebreaker
  // applies: highest commit count wins (insulates primary from git's
  // second-precision `--since`, which can sweep an initial commit into the
  // window when sessionStart was captured in the same second).
  // Capture the actual commit hashes here (not just counts) so Phase 2 can
  // record commits per qualifying repo without re-running findSessionCommits.
  const commitsByRepo = new Map<string, string[]>();
  if (sessionStartTime && repoCandidates.length > 0) {
    await Promise.all(
      repoCandidates.map(async c => {
        try {
          const commits = await findSessionCommits(c.path, sessionStartTime);
          commitsByRepo.set(c.path, commits);
        } catch {
          commitsByRepo.set(c.path, []);
        }
      })
    );
  ...
  // Primary selection:
  //   1. Among qualifying repos at the top score, pick highest commit count.
  //   2. Otherwise, fall back to top-scoring candidate (any qualify status).
  // (1) preserves c4ee373's tied-top tiebreaker; (2) keeps a project link for
  // edits-only sessions with no in-window commits.
  let primary: DetectedRepo | null = null;
  if (qualifyingRepos.length > 0) {
    const topScore = qualifyingRepos[0].score;
    const topTier = qualifyingRepos.filter(c => c.score === topScore);
    primary = topTier.reduce((best, c) =>
      (commitsByRepo.get(c.path)?.length ?? 0) > (commitsByRepo.get(best.path)?.length ?? 0)
        ? c
        : best
    );
  } else if (repoCandidates.length > 0) {
    primary = repoCandidates[0];
  }
  if (primary) {
    detectedRepoInfo = {
      path: primary.path,
...
- **Last Updated:** ${dateStr}${description}`;
  ...
  // Decision 061 step 7: ensure every non-primary qualifying repo also has a
  // project page before Phase 2's recordCommit loop runs. Without this, the
  // first commit recorded against a repo whose project page doesn't exist
  // yet fails silently inside recordCommit (project.md read errors), and the
  // session backlink for that repo is never written.
  ...
  // Decision 061: project slugs of qualifying repos for frontmatter `repositories` field.
  // Primary first; remaining qualifying repos in score-descending order.
  let repositories: string[] | undefined;
  if (qualifyingRepos.length > 0 && primary) {
    const projectsDir = path.join(context.vaultPath, 'projects');
    const orderedRepos = [primary, ...qualifyingRepos.filter(r => r.path !== primary.path)];
    repositories = await Promise.all(
      orderedRepos.map(r => getOrGenerateProjectSlug(r.path, r.remote ?? null, projectsDir))
    );
  }
