...
export interface GetTasksByDateArgs {
  date: string; // 'today' | 'tomorrow' | 'this-week' | YYYY-MM-DD format
...
export async function getTasksByDate(
  ...
  // Parse all task lists in parallel
  ...
  if (allTasks.length === 0) {
    return {
      content: [
        {
          type: 'text',
          text: `No ${status} tasks found for ${targetDate}${project ? ` in project "${project}"` : ''}.`,
        },
      ],
    };
  }
  ...
  // Format output
  let output = `Found ${allTasks.length} ${status} task(s) for ${targetDate}:\n\n`;
